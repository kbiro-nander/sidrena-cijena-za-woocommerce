...
**Compliance defaults (v1.1.1)**: hidden products and third-party product types are listed; mini-cart badge on; on-change regeneration for all products (10 min debounce) with a clamp so pre-08:00 changes publish immediately; a warning when the daily time is after 07:30; outlets carry permanent keys (`outlet.key`, `outlets.additional[].key`) so renaming keeps the archive; sweep-detected changes fire `scwc_price_changed`; `index.json` carries `direct_url` per file.
