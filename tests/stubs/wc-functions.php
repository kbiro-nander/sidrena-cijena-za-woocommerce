...
if ( ! function_exists( 'wc_format_decimal' ) ) {
	function wc_format_decimal( $number, $dp = false, bool $trim_zeros = false ): string {
...
if ( ! function_exists( 'wc_get_price_to_display' ) ) {
	function wc_get_price_to_display( WC_Product $product, array $args = [] ): float {
		return (float) ( $args['price'] ?? $product->get_price() );
	}
}
if ( ! function_exists( 'wc_get_price_including_tax' ) ) {
	function wc_get_price_including_tax( WC_Product $product, array $args = [] ): float {
		return (float) ( $args['price'] ?? $product->get_price() );
	}
}
if ( ! function_exists( 'wc_get_price_excluding_tax' ) ) {
	function wc_get_price_excluding_tax( WC_Product $product, array $args = [] ): float {
		return (float) ( $args['price'] ?? $product->get_price() );
	}
}
if ( ! function_exists( 'wc_format_sale_price' ) ) {
	function wc_format_sale_price( $regular_price, $sale_price ): string {
...
if ( ! function_exists( 'wc_format_price_range' ) ) {
	function wc_format_price_range( $from, $to ): string {
...
if ( ! function_exists( 'wc_bool_to_string' ) ) {
	function wc_bool_to_string( $bool ): string { return wc_string_to_bool( $bool ) ? 'yes' : 'no'; }
}
...
if ( ! function_exists( 'wc_get_logger' ) ) {
	function wc_get_logger() {
		return new class() {
			...
			public function __call( string $name, array $args ): void { $this->entries[] = [ $name, $args[0] ?? '', $args[1] ?? [] ]; }
		};
	}
}
